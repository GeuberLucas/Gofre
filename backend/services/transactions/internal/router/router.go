...
func getTransactionRoutes(s *service.TransactionService) []routes.Route {
	return []routes.Route{
		...
		{Path: "/expense", Method: "GET", HandlerFunc: handler.GetByIdUserExpenseHandler(s), NeedsAuth: true},
		...
		{Path: "/revenue", Method: "GET", HandlerFunc: handler.GetByIdUserRevenueHandler(s), NeedsAuth: true},
